#getToken(user)：模拟登录获取token
# parseUserInfo()里面存配置文件
...
def get_plan_id(user, token: str, sign: str):
    url = "https://api.moguding.net:9000/practice/plan/v3/getPlanByStu"  #定义登录接口
    data = {
        "state": "",
    }

    #headers里面放token等发包需要的内容
    headers2 = {
        'roleKey': 'student',    #rolekey里面放的是student
        "authorization": token,  #Authorization里面放的就是token
        "sign": sign,
        "content-type": "application/json; charset=UTF-8",
        "user-agent": getUserAgent(user)  #设备信息
    }
    res = requests.post(url=url, data=json.dumps(data), headers=headers2)  #模拟登录发包
    return res.json()["data"][0]["planId"]  #将模拟登录获取到的信息存入data


def getUserAgent(user):  #调用配置文件中的设备信息
    if user["user-agent"] != 'null':
        return user["user-agent"]  #若配置文件的设备信息不为null则返回配置文件中的设备信息
    user["user-agent"] = random.choice(  #随机设备信息
        [
    ...
    return user["user-agent"]  #将随机的设备信息返回到配置文件


def getSign2(text: str):  #模拟生成sing值，即先字符串拼接后MD5加密
    s = text + "3478cbbc33f84bd00d75d7dfa69e0daa"
    return md5(s.encode("utf-8")).hexdigest()


def parseUserInfo():  #第一个调用的方法；作用是读取配置文件
    allUser = ''
...
def prepareSign(user, keepLogin=True):  #如果用户开启了保持登录
    if not user["enable"]:  #判断是否开启每日打卡
        return

    if user["keepLogin"] and keepLogin:  #判断keeplogin是否为真（用户是否用了保持登录）
        # 启用了保持登录状态，则使用设备Token登录
        print('用户启用了保持登录，准备使用设备Token登录')
        useUserTokenSign(user)
        return

    userInfo = getToken(user)  #将用户的token信息传给userInfo
    phone = user["phone"]      #电话为user里的phone

    if userInfo["code"] != 200:  #不等于200则打卡失败；code为获取程序运行的结果
...
def signCheck(users):  #每日签到检查
    for user in users:
        if not user["signCheck"] and user["enable"]:
            continue     #如果配置文件中没有开启打卡功能和每日签到检查功能则跳过本次循环

        print("跳过循环")
        url = "https://api.moguding.net:9000/attendence/clock/v1/listSynchro"  #定义签到接口
        if user["keepLogin"]:  #判断是否开启保持登录
            print('          此用户保持登录状态开启，准备使用Token查询          ')
            token = user["token"]  #读取配置文件中的token
        else:
            print('            此用户保持登录状态关闭，准备登录账号          ')
            token = getToken(user)["data"]["token"]  #调用104行的方法模拟登录获取token
        header = {   #调用的设备信息等
        ...
        t = str(int(time.time() * 1000))  #将返回的时间戳转换为字符串然后赋值到“t”
        data = {
            "t": encrypt("23DbtQHR2UMbH6mJ", t)
        }  #用时间戳加密（猜的）
        res = requests.post(url=url, headers=header, data=json.dumps(data))  #执行模拟登录

        if res.json()["msg"] != 'success':   #一般判断结果为真
...
if __name__ == '__main__':
    users = parseUserInfo()  #读取配置文件
    hourNow = datetime.datetime.now(pytz.timezone('PRC')).hour  #现在时间PRC
    # 每日签到检查
    if hourNow == 11 or hourNow == 23:  #如果现在时间为11点或23点
        print('----------------------------每日签到检查开始-----------------------------')
        print('          每日11点以及23点为打卡检查，此时间段内自动打卡不会运行          ')
        try:             #错误处理，是否能正常运行signCheck(users)
            signCheck(users)     #223行
        except Exception as e:   #将错误原因存到e
            print('每日签到检查运行错误！可能与服务器建立连接失败,具体错误原因：' + str(e))
        print('----------------------------每日签到检查完成-----------------------------')
        sys.exit()  #至此每日签到检查结束；退出程序
    for user in users:
        try:  #错误处理，是否能正常运行prepareSign(user)
            prepareSign(user)
        except Exception as e:  #与try一起用，将错误存到e
            print('工学云打卡失败，错误原因：' + str(e))
            isNetworkError = 'HTTPSConnectionPool' in str(e)  #错误信息
            m = '原因请查看下方错误信息，'
            if isNetworkError:
                m = '连接工学云服务器失败！'
            # 错误信息
